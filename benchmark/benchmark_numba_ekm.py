...
def main():
    ...
    res_yes = (
        time_run(X, init_centers, use_numba=True, repeats=repeats)
        if HAVE_NUMBA
        else None
    )
    ...
    header = (
        f"{'Variant':15s} {'Mean(s)':>10s} {'Std(s)':>9s} {'Objective':>12s} {'Hist':>20s}"
        + ("  Durations")
    )
    print(header)
    print("-" * len(header))
    print(
        f"{'No numba':15s} {res_no['mean']:10.4f} {res_no['std']:9.4f} {res_no['objective']:12.4f} {str(res_no['label_hist']):>20s}  {res_no['durations']}"
    )
    if res_yes:
        print(
            f"{'With numba':15s} {res_yes['mean']:10.4f} {res_yes['std']:9.4f} {res_yes['objective']:12.4f} {str(res_yes['label_hist']):>20s}  {res_yes['durations']}"
        )
    ...
    speedup = (
        (res_no["mean"] / res_yes["mean"])
        if (res_yes and res_yes["mean"] > 0)
        else None
    )
    if speedup:
