...
def gen_dirichlet_mixture(
    n_samples: int, k_true: int, dim: int, seed: int, imbalance: float
...
def run(args):
    X, y = gen_dirichlet_mixture(
        args.n_samples, args.n_clusters, args.dim, args.seed, args.imbalance
    )
    ...
    for s in seeds:
        r, artifacts = run_once(
            X, y, args.n_clusters, s, args.silhouette, args.silhouette_subsample
        )
        ...
        if args.verbose:
            print(
                f"seed {s:4d} | ARI km {r.ari_km:.4f} ekm {r.ari_ekm:.4f} | NMI km {r.nmi_km:.4f} ekm {r.nmi_ekm:.4f} | SSE km {r.sse_km:.1f} ekm {r.sse_ekm:.1f} | time km {r.time_km*1e3:.1f}ms ekm {r.time_ekm*1e3:.1f}ms"
            )
    ...
    def agg_pair(a, b, label, larger_better=True):
        ...
        print(
            f"{label:<8} KMeans {ma:.4f}±{sa:.4f} | EKM {mb:.4f}±{sb:.4f} | wins km/ekm {wins_a}/{wins_b} ({direction} better)"
        )

    print(
        "\n==== Summary ({} runs, dim={}, K={}) ====".format(
            len(results), args.dim, args.n_clusters
        )
    )
    ...
    mk, sk = mean_std([r.time_km for r in results])
    me, se = mean_std([r.time_ekm for r in results])
    print(
        f"Time    KMeans {mk*1e3:.2f}±{sk*1e3:.2f} ms | EKM {me*1e3:.2f}±{se*1e3:.2f} ms"
    )
    if args.silhouette:
...
def cluster_plot(X: np.ndarray, y: np.ndarray, km, ekm, labels_km, labels_ekm, args):
    proj_X, proj_model = _project_data(
        X, args.cluster_plot_method, args.cluster_plot_seed
    )
    # Optionally subsample for clarity
    if (
        args.cluster_plot_subsample > 0
        and proj_X.shape[0] > args.cluster_plot_subsample
    ):
        rng = np.random.default_rng(args.cluster_plot_seed)
        idx = rng.choice(
            proj_X.shape[0], size=args.cluster_plot_subsample, replace=False
        )
    else:
    ...
    for ax in axes:
        ...
        ax.set_yticks([])
    fig.suptitle(
        "Cluster Projection ({} -> 2D via {})".format(
            X.shape[1], args.cluster_plot_method.upper()
        )
    )
    fig.tight_layout()
...
def parse_args():
    ap = argparse.ArgumentParser(
        description="High-dimensional Dirichlet imbalance benchmark: KMeans vs EKM"
    )
    ...
    ap.add_argument(
        ...
        type=int,
        default=1,
        help="Monte Carlo repetitions (different seeds)",
    )
    ...
    ap.add_argument(
        ...
        type=float,
        default=50,
        help="Dirichlet boost factor for first cluster",
    )
    ap.add_argument(
        "--silhouette", action="store_true", help="Compute silhouette (may be slow)"
    )
    ap.add_argument(
        "--silhouette-subsample",
        type=int,
        default=4000,
        help="Subsample size for silhouette if dataset bigger",
    )
    ap.add_argument(
        "--plot", action="store_true", help="Generate boxplots if matplotlib available"
    )
    ap.add_argument(
        "--save-plot", type=str, default="", help="Save boxplot figure to file"
    )
    ...
    ap.add_argument(
        ...
        type=bool,
        default=True,
        help="Plot 2D projection (PCA) for one run (first seed) with true labels, KMeans, EKM",
    )
    ap.add_argument(
        "--cluster-plot-method",
        type=str,
    ...
    ap.add_argument(
        "--cluster-plot-subsample",
        type=int,
        default=6000,
        help="Subsample points for cluster plot (0 = no subsample)",
    )
    ap.add_argument(
        "--cluster-plot-seed",
        type=int,
        default=123,
        help="Random seed for subsampling / projection reproducibility",
    )
    ap.add_argument(
        "--cluster-plot-save",
        type=str,
        default="",
        help="Save cluster projection figure",
    )
