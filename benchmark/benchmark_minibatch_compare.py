...
def run_and_report(name, fit_callable, X, y):
    ...
    obj = (
        stable_objective(X, model.cluster_centers_, model.alpha_, model.metric)
...
def main():
    ...
    for r in results:
        print(
            f"{r['name']:18s} {r['time_sec']:8.3f} {r['ARI']:7.3f} {r['NMI']:7.3f} {r['objective']:12.4f} {r['alpha']:9.4f} {str(r['epochs_or_iters']):>10s}  {r['cluster_sizes']}"
        )
    ...
    if _HAVE_PLT:
        ...
        axes[0].scatter(
            results[0]["cluster_sizes"].argmax(),
            results[0]["cluster_sizes"].argmax(),
            alpha=0,
        )  # dummy
        ...
        for ax, model, title in zip(
            axes, [fb, la, lo], ["EKM-full", "MiniBatch-Accum", "MiniBatch-Online"]
        ):
            labels = model.predict(X)
